...
> `pubspec.yaml` `version:` field at release time; the channel itself is version-agnostic.

## [Unreleased]

（目前無待發項目——`2.5.1` 已於下方發布，三套件共用版號 lockstep。）
...
> （additive、非 BREAKING）。兩項獨立修正：① 補齊 `2.0.0`（R29 播放器手勢三度改版）當時刻意
> 記錄的 Non-Goals 延後項——乾淨模式退出鈕像素對齊設計稿；② `CollapsibleLivebuyPlayer`
> in-place 換片同步必填 `onVideoChanged` 回呼＋新增 `openSignal` 機制，parity Android/iOS/RN。
>
> ⚠️ **既有紀錄補正**（本次發版盤點時發現，非本輪造成）：`2.0.0` 對外發布時，這份
> `CHANGELOG.md` 因故仍停留在「Initial mirror repository structure」的佔位內容，未跟著同步
> 更新——下方 `## 2.0.0` 段落是事後補記的正確說明，於 `2.0.1` 發版時一併校正到位（完整對照見
> monorepo 內 [`flutter/CHANGELOG.md`](https://github.com/ariesweng/livebuy-sdk)）。
